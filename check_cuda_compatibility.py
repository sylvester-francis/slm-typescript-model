...
def main():
    ...
    if not gpu_info:
        print("⚠️ Could not retrieve GPU info via nvidia-smi. Ensure the NVIDIA driver is installed and nvidia-smi is on PATH.")
    else:
    ...
    if not torch_info.get("torch_installed"):
        ...
        if torch_info.get("cuda_available"):
            ...
            if arches:
                ...
                print("⚠️ Could not determine compiled CUDA architectures in the torch wheel.")
        else:
            print("⚠️ CUDA not available to PyTorch. This may be due to a missing or incompatible CUDA toolkit.")
    ...
    if gpu_info and torch_info.get("cuda_available"):
        ...
        if gpu_cc != torch_cc:
            compatible = False
            print("\n❌ Compute capability mismatch between driver (" + gpu_cc + ") and torch (" + torch_cc + ").")
            print("   Suggested fix: reinstall a torch wheel that includes sm_" + gpu_cc.replace('.', '') + ".")
    if not torch_info.get("cuda_available"):
        compatible = False
        print("\n❌ PyTorch cannot access CUDA. Ensure the driver matches the CUDA version used to build the torch wheel.")

    if compatible:
        print("\n✅ All checks passed. Your GPU, driver, CUDA runtime, and PyTorch appear compatible.")
    else:
        print("\n⚠️ Compatibility issues detected. Review the messages above for remediation steps.")
